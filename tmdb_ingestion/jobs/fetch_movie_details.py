from __future__ import annotations

import argparse
import asyncio
from datetime import datetime, timezone
from typing import Dict, Any, List

import aiohttp
import pandas as pd
import pyarrow as pa
import pyarrow.parquet as pq
from aiolimiter import AsyncLimiter
from tqdm.asyncio import tqdm_asyncio

from tmdb_ingestion.utils import (
    load_config,
    get_api_key,
    fetch_api_data,
    get_filesystem,
    get_data_dir_path,
    build_fs_path,
    ensure_dir_exists,
)


def run_movie_details(cfg: Dict[str, Any]) -> None:
    """
    Public job entrypoint (sync).
    """
    api_key = get_api_key()

    api_cfg = cfg["api"]
    ingest_cfg = cfg["ingestion"]
    conc_cfg = cfg["concurrency"]
    storage_cfg = cfg.get("storage", {})

    batch_size = ingest_cfg.get("batch_size", 500)

    # --- Filesystem Setup ---
    fs = get_filesystem(cfg)
    data_dir_str = get_data_dir_path(cfg)
    
    movies_dir = build_fs_path(cfg, data_dir_str, "movies")
    details_dir = build_fs_path(cfg, data_dir_str, "movie_details")
    ensure_dir_exists(details_dir, fs=fs)

    # Input: read from partitioned movies files using fs.glob
    # fs.glob returns a list of string paths (e.g. ["gs://bucket/data/movies/movies_2020.parquet"])
    glob_pattern = build_fs_path(cfg, movies_dir, "movies_*.parquet")
    movies_files = sorted(fs.glob(glob_pattern))
    
    if not movies_files:
        raise FileNotFoundError(
            f"No movies_*.parquet files found matching {glob_pattern}. "
            f"Run discover_movies.py first."
        )

    asyncio.run(
        _fetch_details_and_credits(
            movies_files=movies_files,
            api_key=api_key,
            api_cfg=api_cfg,
            conc_cfg=conc_cfg,
            details_dir=details_dir,
            batch_size=batch_size,
            storage_cfg=storage_cfg,
            fs=fs,
            cfg=cfg
        )
    )


async def _fetch_details_and_credits(
    movies_files: List[str],
    api_key: str,
    api_cfg: Dict[str, Any],
    conc_cfg: Dict[str, Any],
    details_dir: str,
    fs: Any,
    cfg: Dict[str, Any],
    batch_size: int = 500,
    storage_cfg: Dict[str, Any] = None,
) -> None:
    """
    Fetch movie details and credits for all discovered movies.
    """
    if storage_cfg is None:
        storage_cfg = {}
    
    base_url = api_cfg["details_url"]
    append_to_response = api_cfg.get("append_to_response", "credits")
    timeout = api_cfg.get("timeout", 30)
    compression = storage_cfg.get("compression", "snappy")

    params = {
        "api_key": api_key,
        "append_to_response": append_to_response,
    }

    limiter = AsyncLimiter(conc_cfg["max_rate"], conc_cfg["time_period"])
    semaphore = asyncio.Semaphore(conc_cfg["semaphore_limit"])

    # Load all movie IDs from partitioned files
    print("Loading movie IDs from partitioned files...")
    all_movie_ids = []
    
    # We can pass the list of files directly to read_parquet if they have same schema
    # But doing loop allows progress/logging per file if needed, keeping simple here:
    for movie_file in movies_files:
        # Pass filesystem=fs to handle GCS paths
        df = pd.read_parquet(movie_file, columns=["id"], filesystem=fs)
        all_movie_ids.extend(df["id"].tolist())

    print(f"Found {len(all_movie_ids)} total movies across {len(movies_files)} files")

    output_file = build_fs_path(cfg, details_dir, "movie_details.parquet")

    # Remove old output file if it exists (fresh start)
    if fs.exists(output_file):
        try:
            fs.rm(output_file)
            print(f"Removed existing output file: {output_file}")
        except Exception as e:
            print(f"Warning: Could not remove existing file {output_file}: {e}")

    writer = None  # PyArrow ParquetWriter

    async with aiohttp.ClientSession() as session:
        num_movies = len(all_movie_ids)
        num_batches = (num_movies + batch_size - 1) // batch_size

        print(f"Fetching details in {num_batches} batches...")

        for batch_idx, batch_start in enumerate(range(0, num_movies, batch_size), start=1):
            batch_ids = all_movie_ids[batch_start : batch_start + batch_size]

            tasks = []
            for movie_id in batch_ids:
                url = f"{base_url}{movie_id}"
                task = _fetch_with_metadata(
                    url=url, session=session, params=params,
                    semaphore=semaphore, limiter=limiter,
                    movie_id=movie_id, timeout=timeout,
                )
                tasks.append(task)

            batch_results = await tqdm_asyncio.gather(
                *tasks, total=len(tasks), desc=f"Batch {batch_idx}/{num_batches}", leave=False,
            )

            df_batch = pd.DataFrame(batch_results)
            table = pa.Table.from_pandas(df_batch)

            if writer is None:
                # Initialize ParquetWriter with filesystem object
                writer = pq.ParquetWriter(
                    output_file,
                    table.schema,
                    compression=compression,
                    filesystem=fs
                )

            writer.write_table(table)
            print(f"Batch {batch_idx}/{num_batches} complete ({len(df_batch)} movies)")

    if writer is not None:
        writer.close()
        print(f"Wrote all movie details to {output_file}")


async def _fetch_with_metadata(
    url: str,
    session: aiohttp.ClientSession,
    params: Dict[str, Any],
    semaphore: asyncio.Semaphore,
    limiter: AsyncLimiter,
    movie_id: int,
    timeout: int = 30,
) -> Dict[str, Any]:
    data = await fetch_api_data(
        url=url, session=session, params=params,
        semaphore=semaphore, limiter=limiter, timeout=timeout,
    )
    return {
        "movie_id": movie_id,
        "payload_json": data,
        "ingested_at": datetime.now(timezone.utc),
    }

def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument("--batch-size", type=int)
    return parser.parse_args()

if __name__ == "__main__":
    cfg = load_config()
    args = _parse_args()
    if args.batch_size is not None:
        cfg["ingestion"]["batch_size"] = args.batch_size
    run_movie_details(cfg)